...
export default function FeaturedProject() {
  return (
    <section className="py-24 md:py-32 mx-auto max-w-[1600px] px-6 md:px-12">
      {siteConfig.featuredCaseStudies.map((cs, index) => {
        const project = siteConfig.projects.find((p) => p.id === cs.projectId);
        if (!project) return null;

        const arch = (
          siteConfig.architectures as Record<string, readonly string[]>
        )[project.id];
        ...
        return (
          <div key={project.id} className={index > 0 ? "hairline-top mt-24 pt-24 md:mt-32 md:pt-32" : ""}>
            <Reveal>
